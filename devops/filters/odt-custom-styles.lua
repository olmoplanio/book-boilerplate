...
local unnumberedStyle = 'Text_20_body'
...
function getParaTags(style)
  ...
  -- sanitize style names to ensure well-formed XML attributes
  local function sanitize_style(s)
    if not s then return '' end
    -- remove outer braces and common template markers
    s = tostring(s)
    -- remove any double or single quotes
    s = s:gsub('"', ''):gsub("'", '')
    -- strip characters that are unsafe in style names, allow letters, numbers, space, dash, underscore, colon
    s = s:gsub('[^%w% %-%_%:]', '')
    -- trim
    s = s:match('^%s*(.-)%s*$') or s
  ...
  local safe = sanitize_style(style)
  local startTag = '<text:p text:style-name="' .. safe .. '">'
  local endTag = '</text:p>'
  ...
  if FORMAT == 'odt' and sp.attr and sp.attr.attributes then
    ...
    if customStyle then
      sp = pandoc.walk_inline(sp, util.inlineToRaw)
        ...
        local safe = sanitize_style(customStyle)
        local startTag = '<text:span text:style-name="' .. safe .. '">'
...
function Header (hx)
  if FORMAT == 'odt' and hx.attr and hx.attr.classes[1] then
    ...
    hx = pandoc.walk_block(hx, util.blockToRaw)
      ...
      local safeclass = sanitize_style(class)
      local startTag = '<text:h text:style-name="' .. safeclass .. '" text:outline-level="' .. hx.level .. '">'
    local endTag = '</text:h>'
